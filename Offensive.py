...
df_minority = df[df['class'] == min_class]
df = pd.concat([df] + [df_minority] * (class_counts.max() // class_counts.min() - 1), ignore_index=True)
...
# Epoch'lara göre doğruluk grafiği
def plot_epoch_accuracies(accuracies):
    epochs, accs = zip(*accuracies)
    plt.figure(figsize=(10, 6))
    plt.plot(epochs, accs, marker='o', linestyle='-', color='b')
    plt.xlabel('Epoch')
    plt.ylabel('Doğruluk (%)')
    plt.title('Epochlara Göre Doğruluk')
    plt.grid()
    plt.show()

plot_epoch_accuracies(accuracies)

# Eğitim ve doğrulama doğruluğu çizgileri
def plot_training_validation_accuracy(history):
    plt.figure(figsize=(10, 6))
    plt.plot(history['train_acc'], label='Eğitim Doğruluğu', marker='o', linestyle='-')
    plt.plot(history['val_acc'], label='Doğrulama Doğruluğu', marker='o', linestyle='-')
    plt.xlabel('Epochs')
    plt.ylabel('Doğruluk (%)')
